...
for $trip at $count in doc('/db/apps/travels/president-travels/obama-barack-2015.xml')//trip
let $stamp := 
    (
...
let $update := update insert element id {()} preceding $trip/date
let $update := update insert $stamp following $trip/remarks
let $update := update insert (<role>president</role>, <name>Barack Obama</name>) following $trip/id
let $update := update insert (attribute who {'obama-barack'}, attribute role {'president'}) into $trip
return $trip
...
for $trip in doc('/db/apps/travels/president-travels/obama-barack-2015.xml')//trip
